...
class SSMLKVForestPredictor(BaseEstimator, ClassifierMixin):
    ...
    alpha = 10, C = 0.01,  gamma = 10, eta = 0.5, hyper_params_dict = None, do_ranking_split = False, p=[.5,0,0,.5], q=[1,0,0], use_complex_model=False, njobs=None):
        super().__init__()
        self.njobs = njobs
    ...
    def get_label_example_balance_score(self, labels_df):
        ...
        for name,col in labels_df.items():
            ...
            if( (0 in value_counts.index.array) and (1 in value_counts.index.array) ):
                ...
                g = min_obs/max_obs
                avg += p*g # is the labeling proportion times labeling degree between them. We are looking for the bogger balanced one
            else:
                avg += 0
                continue
    ...
    def generate_childrenV1_1(self, node,recursion, tree_id = 0 , full_instances=None, full_labels=None):
        ...
        while m < m_groups and iteration < max_iterations:
            ...
            if( do_spectral_clustering ):
                ...
                # eigen tol modified according to documentation...
                
                spectral_model = SpectralClustering(n_clusters = 2, eigen_solver='amg' , eigen_tol=1e-5 , assign_labels='cluster_qr', gamma = self.node_hyper_params["gamma"] , n_jobs=2)
                #spectral_model = SpectralClustering(n_clusters = 2, affinity='nearest_neighbors' , assign_labels='cluster_qr')
                ...
                with warnings.catch_warnings():
                    try:
                        ...
                        #A = 1 - euclidean_distances( instances[ selected_cols ].to_numpy()  ,squared=True)
            ...
            if( not do_spectral_clustering):
                ...
                left_supervised_instances = list(set(best_col[ best_col <= best_split_val ].index.array).intersection(supervised_index))
                right_supervised_instances = list(set(best_col[ best_col > best_split_val ].index.array).intersection(supervised_index))   
                ...
                label_balance_score_left = self.get_label_balance_score(labels.loc[list(left_supervised_instances)])
                label_balance_score_right = self.get_label_balance_score(labels.loc[list(right_supervised_instances)])
                ...
                s_score = 0
                if( self.node_hyper_params["pB"] > 0.000000001):
                ...
                # # idea 2 (2023-05-25) we need examples positive and negative from all the labels. Try to do this instead
                # of penalizing just of the less supervised nodes. It might be better to have fewer supervised nodes
                # but with positives and negative examples of every label. In this case, a good number to have is
                # pow(2,number_of_labels) as this would integrate at least one example of each combination... but
                # it could happen that two instances have labels like : 010101 and 101010 , so we could have positive and negative
                # examples of each label with only 2 instances.  
                ...
                s_score = 0
                if( self.node_hyper_params["pB"] > 0.00001): # avoid heavy computation
